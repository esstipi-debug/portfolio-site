...
- Identical header/footer/nav markup is intentionally repeated verbatim in every one of the 5 HTML pages, rather than factored into a shared include — this is a deliberate spec decision (design spec §4: "content is duplicated in the shared header/footer markup across 5 pages. Acceptable at this scale... if the site grows meaningfully beyond this, a static-site generator becomes worth the added tooling, but YAGNI for v1"), not a DRY violation to flag or fix.
